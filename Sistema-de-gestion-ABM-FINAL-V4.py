...
class Ventas(Cliente):
    ...
    def menu(self):
        ...
        if opc == 1:
            ...
            if tipoCliente == 1:
                self.dniCliente = int(input("Ingrese el DNI 9000 - 99999: "))
                self.fecha = date.today()
                self.codigoBarra = input("Ingrese el Codigo de Barra: ")
                ...
                cursor.execute ("UPDATE articulos SET Stock = Stock - %s WHERE Codigo_Barra = %s", (self.cantidad, self.codigoBarra))
                miConexion.commit()
                # Agregar el producto a la tabla Ventas
             ...
             # verificar si el DNI existe, si no existe crearlo
                ...
                if resultado:
                 ...
                 for i in resultado:
                     ...
                     print("El nombre del producto es: ", self.nombre)
                        #print(i)
                 # Si ya existe el DNI, actualizar el monto de la venta , si no existe, crearlo
                 cursor.execute ( "SELECT * FROM Ventas WHERE DNI_Cliente = %s", (self.dniCliente,))
                 resultado = cursor.fetchall()
                 if resultado:
                        cursor.execute ("UPDATE Ventas SET Monto = Monto + %s WHERE DNI_Cliente = %s", (self.monto, self.dniCliente))
                        miConexion.commit()
                        # Agregar el nombre del producto a la tabla Ventas
                        cursor.execute ("UPDATE Ventas SET Productos = Productos + %s WHERE DNI_Cliente = %s", (self.nombre, self.dniCliente))
                        miConexion.commit()
                        print("Venta realizada, Base de Datos Actualizada")
                        print("Factura Generada")
                        print("##############")
                 ...
                 os.system("pause")
                 os.system("cls")
                 Ventas.menu() #  o probar con Ventas.menu(self)
